...
export async function renderSettings(rootEl) {
  ...
  rootEl.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="muted small">Just colors for now.</div>
    ...
    <div class="card">
      ...
      <div class="muted small">Export JSON. Import restores everything (Replace All).</div>
      <div class="divider"></div>
    ...
    <div class="card">
      ...
      <div class="muted small">Use <b>MM-DD</b>. Shows as <span class="pinkDot"></span> in calendar every year.</div>
      <div class="divider"></div>

      <div class="row" style="gap:10px; flex-wrap:wrap;">
        <input class="input" id="holidayMD" placeholder="MM-DD (e.g. 10-31)" style="max-width:210px;" />
      ...
      <div class="divider"></div>
      <div id="holidayList">${renderHolidayList(holidays)}</div>
    </div>
    ...
    <div class="card">
      ...
      <div class="muted small">Debug is off unless you enable it.</div>
      <div class="divider"></div>
  ...
  // Add custom holiday (MM-DD)
  rootEl.querySelector("#btnAddHoliday").addEventListener("click", async () => {
    const md = (rootEl.querySelector("#holidayMD").value || "").trim();
    const name = (rootEl.querySelector("#holidayName").value || "").trim();

    if (!/^\d{2}-\d{2}$/.test(md)) return toast(rootEl, "Use MM-DD (e.g. 10-31)");
    if (!name) return toast(rootEl, "Enter a name");

    await addCustomHoliday(md, name);
    await debugLog(`settings:addHoliday ${md}`);
    await renderSettings(rootEl);
  });
  ...
  rootEl.querySelectorAll("[data-hedit]").forEach(btn => {
    btn.addEventListener("click", async () => {
      openEditHolidaySheet(
        rootEl,
        btn.getAttribute("data-hedit"),
        btn.getAttribute("data-md"),
  ...
  rootEl.querySelector("#btnCopyDebug").addEventListener("click", async () => {
    const text = (await getDebugLog()).join("\n");
...
function renderHolidayList(list) {
  if (!list.length) return `<div class="muted">No custom holidays yet.</div>`;
  return [...list]
      ...
      <div class="hRow">
        <div class="hLeft">
          ...
          <div>
            ...
            <div class="muted small">${escapeHTML(h.md)}</div>
          </div>
        </div>
        <div class="row" style="gap:8px;">
          <button class="btn" data-hedit="${h.id}" data-md="${escapeAttr(h.md)}" data-name="${escapeAttr(h.name)}" type="button">Edit</button>
...
function openEditHolidaySheet(rootEl, id, md, name) {
    ...
    <div class="sheetPanel">
      <div class="sheetHeader">
        <div>
          ...
          <div class="muted small">MM-DD + name</div>
        </div>
      ...
      <input class="input" id="editMD" value="${escapeAttr(md)}" />
  ...
  host.querySelector("#btnSave").onclick = async () => {
    const newMD = (host.querySelector("#editMD").value || "").trim();
    const newName = (host.querySelector("#editName").value || "").trim();
    if (!/^\d{2}-\d{2}$/.test(newMD)) return alert("Use MM-DD");
    if (!newName) return alert("Name required");
    await updateCustomHoliday(id, { md: newMD, name: newName });
    close();
    await renderSettings(rootEl);
  };
...
function applyAccentCSS(accentKey) {
  const map = {
    red: "#ef4444", ember: "#f97316", orange: "#f59e0b", pink: "#ec4899",
    purple: "#a855f7", blue: "#3b82f6", green: "#22c55e"
  };
  document.documentElement.style.setProperty("--accent", map[String(accentKey||"red").toLowerCase()] || map.red);
}
...
function safePreviewBackup(jsonText) {
  try {
    ...
    return {
      ok: !!(data.tasks && data.templates && data.settings),
      data: {
...
function injectStylesOnce() {
  ...
  style.textContent = `
    ...
    .chip{ padding:10px 12px; border-radius:14px; border:1px solid rgba(255,255,255,.12); background: rgba(255,255,255,.04); font-weight:900; }
    .fileBtn{ display:inline-flex; align-items:center; justify-content:center; padding:12px 14px; border-radius:14px; border:1px solid rgba(255,255,255,.12); background: rgba(255,255,255,.04); font-weight:900; position:relative; overflow:hidden; }
    .fileBtn input{ position:absolute; inset:0; opacity:0; cursor:pointer; }
    .previewBox{ margin-top:12px; padding:12px; border-radius:14px; border:1px solid rgba(255,255,255,.12); background: rgba(255,255,255,.03); }
    .hidden{ display:none; }
    .pinkDot{ display:inline-block; width:8px;height:8px;border-radius:99px; background:#ec4899; margin-right:8px; transform: translateY(-1px); }
    .hRow{ display:flex; align-items:center; justify-content:space-between; gap:12px; border:1px solid rgba(255,255,255,.10); background: rgba(255,255,255,.03); border-radius:14px; padding:12px; margin-bottom:10px; }
    .hLeft{ display:flex; align-items:center; gap:12px; }
    .btn--danger{ background: rgba(239,68,68,.18); border-color: rgba(239,68,68,.30); }
    .sheetBackdrop{ position:fixed; inset:0; background: rgba(0,0,0,.55); z-index:9998; }
    .sheetPanel{ position:fixed; left:50%; transform: translateX(-50%); bottom:14px; width:min(720px, calc(100vw - 24px)); background: rgba(20,20,22,.94); border:1px solid rgba(255,255,255,.12); border-radius:18px; padding:12px; z-index:9999; backdrop-filter: blur(14px); max-height: calc(100vh - 120px); overflow:auto; }
    .sheetHeader{ display:flex; justify-content:space-between; align-items:flex-start; gap:12px; }
    .sheetTitle{ font-weight:900; font-size:16px; letter-spacing:-0.2px; }
    .toast{ position:fixed; top:14px; left:50%; transform: translateX(-50%); padding:10px 14px; border-radius:999px; background: rgba(255,255,255,.08); border:1px solid rgba(255,255,255,.12); backdrop-filter: blur(12px); z-index:10000; font-weight:900; }
  `;
