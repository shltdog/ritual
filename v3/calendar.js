// Ritual V3 — calendar.js (FINAL)
// 7-column calendar grid + US holidays (federal + common) + task indicator
// Offline, no APIs.
...
export async function renderCalendar(rootEl) {
  ...
  // Build holiday map for the displayed year
  ...
  rootEl.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="muted small" id="legend" style="display:flex;gap:12px;flex-wrap:wrap;">
  ...
  rootEl.querySelector("#calPrev").addEventListener("click", async () => {
    ...
    // if selected is not in new month, select first day
    const firstKey = dateKeyFromDate(new Date(renderCalendar.state.year, renderCalendar.state.month, 1));
    renderCalendar.state.selected = firstKey;
...
// US holidays: federal + common ones people expect
function getUSHolidayMap(year) {
  const map = new Map();

  // Helper to set
  const set = (dateObj, name) => map.set(dateKeyFromDate(dateObj), name);

  // Federal (observed where applicable)
